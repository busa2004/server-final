...
public interface ReportRepository extends JpaRepository<Report, Long> {
	...
	@Query("select NEW com.douzone.df.payload.ReportResponse(a.id,a.title,a.content,a.userTask.task.title as taskTitle,a.userTask.user.name as userName,a.createdAt,a.status,a.fileName) "
			+ "from Report a, UserTask b ,UserTask c "
			+ "where b.id=a.userTask.id AND c.id =?1 AND c.task.id = b.task.id AND (a.status = 'WAIT' OR a.status = 'PROGRESS') AND a.open=1 ORDER BY a.createdAt desc")
	List<ReportResponse> getByUserTaskId(Long userTaskId, PageRequest pageRequest);
