...
public interface ReportRepository extends JpaRepository<Report, Long> {
	...
	@Query("SELECT v FROM Report v "
	 		+ "WHERE  v.title LIKE %?1% and v.createdAt >= date(?2) and v.createdAt <= date_format(?3, '%Y-%m-%d %h:%i:%s') and v.userTask.user.id = ?4 ORDER BY v.createdAt DESC")
	...
	@Query("SELECT v FROM Report v "
	 		+ "WHERE  v.title LIKE %?1% and v.createdAt >= date(?2) and v.createdAt <= date_format(?3, '%Y-%m-%d %h:%i:%s') and v.userTask.id =?5 and v.userTask.user.id = ?4 ORDER BY v.createdAt DESC")
	List<Report> findAllById(String search, String from, String to, Long id, Long taskId);
	 @Query("SELECT NEW com.douzone.df.payload.ReportResponse(v.id,v.title,v.content,v.userTask.task.title as taskTitle,v.userTask.user.name as userName,v.createdAt,v.status,v.fileName) FROM Report v "
		 		+ "WHERE  v.status = ?1 and  v.title LIKE %?2% and v.createdAt >= date(?3) and v.createdAt <= date_format(?4, '%Y-%m-%d %h:%i:%s') and v.userTask.task.id = ?5 ORDER BY v.createdAt DESC")
